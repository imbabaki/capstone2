...
<body>
    ...
    <div class="order-summary">
      ...
      <div class="order-details">
        ...
        <div class="detail-row">
          ...
          <span class="detail-value">{{ ucfirst($order['color'] ?? 'grayscale') }}</span>
        </div>
        <div class="detail-row">
      ...
      <div class="payment-status">
        <div class="amount-display">
          <div class="amount-row">
            ...
            <span class="amount-value total">₱{{ number_format($order['total'] ?? 0, 2) }}</span>
          </div>
          <div class="amount-row">
        ...
        <div id="remaining-msg" class="remaining-message">
          💰 Please insert ₱{{ number_format(($order['total'] ?? 0) - ($coinTotal ?? 0), 2) }}
        </div>
  ...
  <script>
    ...
    function processPayment() {
      ...
      fetch("{{ route('usbfd.payment.handle') }}", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          "X-CSRF-TOKEN": "{{ csrf_token() }}",
      ...
      .then(data => {
        ...
        if (data.success || data.redirect) {
          ...
          setTimeout(() => {
            window.location.href = data.redirect || "{{ route('usbfd.instruction') }}";
          }, 1000);
        } else {
    ...
    // Connect to Flask SSE endpoint for live coin updates with fallback
    ...
    function connectCoinStream() {
      ...
      evtSource.onmessage = function(event) {
        ...
        try {
          ...
          // Update the payment display with real coin data
          updatePayment(total);
    ...
    // Apply voucher button - if exists
    const applyVoucherBtn = document.querySelector('.voucher-btn');
    if (applyVoucherBtn) {
