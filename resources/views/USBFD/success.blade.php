...
<body>
        ...
        <div class="right-section">
            ...
            <div class="receipt-card">
                ...
                <div class="receipt-row">
                    ...
                    <span class="receipt-value">{{ basename($order['file_path'] ?? 'N/A') }}</span>
                </div>
                ...
                <div class="receipt-row">
                    ...
                    <span class="receipt-value">{{ ucfirst($order['color_option'] ?? $order['color'] ?? 'N/A') }}</span>
                </div>
                ...
                <div class="receipt-row receipt-total">
                    ...
                    <span class="receipt-value">₱{{ number_format($order['total'] ?? $order['calculated_total'] ?? 0, 2) }}</span>
                </div>
                @endif
            </div>

            <!-- Change Voucher (if exists) -->
            @if(isset($order) && (!empty($order['voucher_code']) || !empty($order['change_voucher_code'])))
            <div class="voucher-card">
                <div class="voucher-title">🎟️ YOUR CHANGE VOUCHER</div>
                <div class="voucher-code-box">
                    <div class="voucher-code">{{ $order['voucher_code'] ?? $order['change_voucher_code'] }}</div>
                    <div class="voucher-amount">₱{{ number_format($order['change_amount'] ?? 0, 2) }}</div>
                </div>
                @if(!empty($order['voucher_expires_at']) || !empty($order['change_voucher_expires_at']))
                <div class="voucher-note">
                    Expires: {{ $order['voucher_expires_at'] ?? $order['change_voucher_expires_at'] }}
                </div>
                @endif
    ...
    <script>
        ...
        console.log('Order details:', @json($order ?? []));
    </script>
