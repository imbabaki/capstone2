...
<head>
  ...
  <title>QR Upload Preview - Instaprint</title>
...
<body>
  ...
  <div class="main-container">
    <h1>QR UPLOAD FILE PREVIEW & SETTINGS</h1>
      ...
      <div class="options">
        ...
        <form action="{{ route('upload.payment') }}" method="POST">
          ...
          <select id="paper_size" name="paper_size">
            ...
            <option value="Letter" {{ ($order['paper_size'] ?? '')=='Letter' ? 'selected' : '' }}>Short</option>
            <option value="Legal" {{ ($order['paper_size'] ?? '')=='Legal' ? 'selected' : '' }}>Long</option>
          </select>

          <label for="copies">Number of Copies</label>
          <div class="number-input-wrapper">
            <input type="number" id="copies" name="copies" value="{{ $order['copies'] ?? 1 }}" min="1" readonly>
          ...
          <input type="text" id="pages" name="pages" value="{{ $order['pages'] ?? '' }}" placeholder="All pages" readonly>

          <label for="color_option">Color Mode</label>
          <select id="color_option" name="color_option">
            <option value="color" {{ ($order['color'] ?? $order['color_option'] ?? '')=='color' ? 'selected' : '' }}>W/Color</option>
            <option value="grayscale" {{ ($order['color'] ?? $order['color_option'] ?? '')=='grayscale' ? 'selected' : '' }}>Black&White</option>
          </select>

          <!-- Hidden fields for duplex and fit -->
          <input type="hidden" id="duplex" name="duplex" value="one-sided">
